...
impl EncryptionAttack for PohligHellmanAttack {
    ...
    fn run(&self, cancel: Arc<AtomicBool>, seed: u64, public_data: EncryptionPublicData) -> AttackReport {
        ...
        let secret_key = match Self::find_secret_key(&modulus, &generator, &key) {
            Some(v) => v,
            None => return make_report(0, AttackResult::Failed { reason: String::from("Не удалось найти секретный ключ") }),
        };
        let decoded_message = Self::decode(ciphertext, &secret_key, &modulus);
        make_report(0, AttackResult::Success { message: decoded_message })
    }
}
...
impl PohligHellmanAttack {
    ...
    fn find_secret_key(modulus: &BigUint, generator: &BigUint, key: &BigUint) -> Option<BigUint> {
        let p_minus_1 = modulus - BigUint::one();

        let factors_p_minus_1 = Self::factorize_prime_powers(&p_minus_1);
        println!("factors(p-1) = {:?}", factors_p_minus_1);

        let order = Self::multiplicative_order(generator, modulus, &factors_p_minus_1);
        println!("ord(g) = {}", order);

        let factors_order = Self::factorize_prime_powers(&order);
        println!("factors(ord(g)) = {:?}", factors_order);

        let mut congruences = Vec::new();

        for (q, e) in factors_order {
            println!("Solving for q = {}, e = {}", q, e);
    ...
    fn crt(congruences: &[(BigUint, BigUint)]) -> Option<BigUint> {
        ...
        for (remainder, modulus) in congruences {
            ...
            let inverse = match modinv(&(partial_modulus.clone() % modulus).to_bigint().unwrap(), &modulus.to_bigint().unwrap()) {
                Some(v) => v,
                None => return None,
            };
    ...
    fn factorize_prime_powers(n: &BigUint) -> Vec<(BigUint, u32)> {
        // Если n раскладывается в большое количество маленьких простых чисел, то атака будет успешной
    ...
    fn solve_prime_power_residue(
        ...
        for i in 0..e {
            ...
            let a_i = Self::small_dlog_bruteforce(&gamma, &h, p, q)?;
